...
$(function () {
    ...
    if ($("#background-list-container").length) {
        //* -------- STORE -------------

        var backFormStore = $("#registerBackgroundImageForm");
        var inputbackImageStore = backFormStore.find(".input-background-image");

        initImageChange(inputbackImageStore, backFormStore, Toast);

        var registerBackForm = $("#registerBackgroundImageForm").validate({
            rules: {
                image: {
                    required: true,
                },
            ...
            submitHandler: function (form, event) {
                ...
                var form = $(form);
                var loadSpinner = form.find(".loadSpinner");
                var modal = $("#RegisterBackgroundImageModal");
                var img_holder = form.find(".img-holder");
                ...
                $.ajax({
                    ...
                    dataType: "JSON",
                    success: function (data) {
                        var logo_list = $("#background-list-container");
                        // var logo_sidebar = $(".dynamic-logo-container");
                        ...
                        inputbackImageStore.val("");
                        registerBackForm.resetForm();
        ...
        $("html").on("click", ".delete-background-img-btn", function () {
            var url = $(this).data("url");
    ...
    if ($("#editGroupForm").length) {
        let editGroupForm = $("#editGroupForm").validate({
            rules: {
                link_group_wts: {
                    url: true,
                },
    ...
    if ($("#p-banners-list-container").length) {
        ...
        InitAjaxSelect2(
            ".p_banner_order_select",
            P_BANNER_ORDER_SELECT_CONFIG,
            "publishing_order"
        );
        ...
        $("#registerPrincipalBannerModal").on("show.bs.modal", function () {
            pBannerSmntEle.summernote(
                getSummernoteConfig(pBannerSmntEle, registerPBannerForm)
            );
        ...
        $("html").on("click", ".edit-p-banner-btn", function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    form.find(".p_banner_order_select").select2(
                        "trigger",
                        "select",
                        {
                            data: {
                                id: banner.publishing_order,
                                text: banner.publishing_order,
                            },
                        }
                    );

                    if (banner.status == 1) {
                        form.find(".status_p_banner_checkbox").prop(
                            "checked",
                            true
                        );
    ...
    if ($("#sliderImages-list-container").length) {
        ...
        $("html").on("click", ".edit-sliderImage-btn", function () {
            var modal = $("#editSliderImageModal");
            var getDataUrl = $(this).data("send");
            var url = $(this).data("url");
    ...
    if ($("#banners-list-container").length) {
        ...
        InitSelect2(
            ".editOrderClassroomBannerSelect",
            ODDER_SELECT_CLASSROOM_BANNER_CONFIG
        );
        ...
        let summernoteEditPrivacyElement = $("#privacy-policies-content");

        var editPrivacyPoliciesForm = $("#editPrivacyPoliciesForm").validate({
            rules: {
                privacy_policies: {
                    required: true,
                },
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.success) {
                            ...
                            $("#editPrivacyPoliciesForm").html(html);

                            let summernoteEditPrivacyElement = $(
                                "#privacy-policies-content"
                            );

                            summernoteEditPrivacyElement.summernote("destroy");

                            summernoteEditPrivacyElement.summernote(
                                getSummernoteConfig(
                                    summernoteEditPrivacyElement,
                                    editPrivacyPoliciesForm
                                )
                            );
        ...
        let summernoteEditTermsConditionsElement = $(
            "#terms-conditions-content"
        );

        var editTermsConditionsForm = $("#editTermsConditionsForm").validate({
            rules: {
                terms_conditions: {
                    required: true,
                },
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.success) {
                            ...
                            $("#editTermsConditionsForm").html(html);

                            let summernoteEditTermsConditionsElement = $(
                                "#terms-conditions-content"
                            );

                            summernoteEditTermsConditionsElement.summernote(
                                "destroy"
                            );

                            summernoteEditTermsConditionsElement.summernote(
                                getSummernoteConfig(
                                    summernoteEditTermsConditionsElement,
        ...
        let summernoteEditDataDeletionElement = $("#data-deletion-content");

        var editDataDeletionForm = $("#editDataDeletionForm").validate({
            rules: {
                data_deletion: {
                    required: true,
                },
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.success) {
                            ...
                            $("#editDataDeletionForm").html(html);

                            let summernoteEditDataDeletionElement = $(
                                "#data-deletion-content"
                            );

                            summernoteEditDataDeletionElement.summernote(
                                "destroy"
                            );

                            summernoteEditDataDeletionElement.summernote(
                                getSummernoteConfig(
                                    summernoteEditDataDeletionElement,
                                    editDataDeletionForm
                                )
                            );
