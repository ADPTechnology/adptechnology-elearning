...
function getDetailPlan($plan)
{
    if ($plan->duration_type === 'months') {
        return "{$plan->duration} meses";
    } elseif ($plan->duration_type === 'days') {
        return "{$plan->duration} días";
    }
}
...
function verifyItemInShoppingSession($plan)
{
    $cart = collect(Session::get('cart', []));

    if ($cart->isNotEmpty()) {
        $planIds = $cart->pluck('buyable_id');
        $existsInCart = $planIds->contains($plan->id);
...
function getPriceWithCouponInAd($ad)
{
    $coupon = $ad->coupon;
    $plan = $ad->plan;

    if ($coupon->amount_type === 'MONEY') {
        return $plan->price - $coupon->amount < 0 ? 0.0 : $plan->price - $coupon->amount;
    } elseif ($coupon->amount_type === 'PERCENTAGE') {
        return $plan->price - (($plan->price * $coupon->amount) / 100) < 0 ? 0.0 :
            $plan->price - (($plan->price * $coupon->amount) / 100);
    }
}
...
function normalizeInputStatus($data)
{
    ...
    $data['especify_courses'] = isset($data['especify_courses']) ? 'S' : 'N';

    $data['especify_users'] = isset($data['especify_users']) ? 'S' : 'N';
...
{
    $config = Config::select('whatsapp_number', 'whatsapp_message', 'link_group_telegram', 'link_group_wts', 'message_hover_whatsapp', 'text_whatsapp')->first();
...
{
    return request()->routeIs('home.information.course');
}
...
function getAmountDiscount($totalPrice)
{
    ...
    if ($coupon) {
        ...
        if ($coupon->amount_type == 'MONEY') {
            ...
            $discount += ($coupon->amount * $totalPrice) / 100;
        }
...
function getCoupon()
{
    ...
    if ($coupon->amount_type == 'MONEY') {
        $text .= "$ {$coupon->amount} - {$coupon->code}";
    } elseif ($coupon->amount_type == 'PERCENTAGE') {
        $text .= "{$coupon->amount}% - {$coupon->code}";
    }
...
function getAmountToOrder($total, $order)
{
    $discount = 0;
    $coupon = NULL;

    if ($order->coupon) {
        $coupon = Coupon::where('code', $order->coupon->code)->first();
    }
...
function getDiscountCoupon($totalPrice)
{
    $couponCode = Session::get('coupon', false);
    $discount = 0.0;
...
function getCouponToOrder($order)
{
    if (!$order->coupon) {
        return '-';
    } else {
        if ($order->coupon->amount_type == 'MONEY') {
            return "{$order->coupon->code} - ($ {$order->coupon->amount})";
        } elseif ($order->coupon->amount_type == 'PERCENTAGE') {
            return "{$order->coupon->code} - ({$order->coupon->amount}%) ";
        }
    }
}
...
function getSessionEmail()
{
    ...
    $step = Session::get("stepEmail", $default);
    return $step;
}

function  getBorderToTable($key, $keys)
{
    return in_array($key, $keys) ? 'border-bottom' : '';
}

// function getLastSubscription()
// {
//     $user = Auth::user();
//     return $user->subscriptions()->get()->last();
// }
...
//     if ($lastSubscription) {
//         return Carbon::parse($lastSubscription->end_date, 'America/Lima')->startOfDay() >= Carbon::now('America/Lima')->startOfDay();
//     } else {
//         return false;
//     }
// }

function getLastSubscription($course)
{
    return $course->plans->flatMap(function ($plan) {
        return $plan->subscription;
    })->sortBy('created_at')->last();
}

function getActiveSubscription($course)
{

    if (getLastSubscription($course)) {
        $lastSubscription = getLastSubscription($course);
        return Carbon::parse($lastSubscription->end_date, 'America/Lima')->startOfDay() >= Carbon::now('America/Lima')->startOfDay();
    } else {
        return false;
    }
...
function hasOneSubscription()
{
    $user = Auth::user();
    return $user->subscriptions()->count() > 0 ? true : false;
}
