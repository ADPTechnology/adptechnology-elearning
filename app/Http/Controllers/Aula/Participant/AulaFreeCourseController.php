...
class AulaFreeCourseController extends Controller
{
    ...
    public function showChapter(Course $course, SectionChapter $current_chapter)
    {
        $user = Auth::user();
        $user->load([
            'productCertifications' => fn($q) =>
            $q->where('certificable_type', Course::class)
                ->where('certificable_id', $course->id)
        ]);

        $productCertification = $user->productCertifications->first() ?? null;

        if (($productCertification->status ?? null) != 'approved') {
            abort(401);
        }
        ...
        if ($course->active == 'S') {
            ...
            $allProgress_first = $user->progressChapters()->whereHas('courseSection', function ($query) use ($course) {
                $query->where('course_id', $course->id);
            })
                ->select(
                    'section_chapters.id',
            ...
            $current_chapter = $allProgress_first->where('id', $current_chapter->id)->first();
            ...
            $allProgress = $user->progressChapters()->whereHas('courseSection', function ($query) use ($course) {
                ...
                ->select(
                    ...
                    'section_chapters.chapter_order',
                    'section_chapters.content',
                    'section_chapters.created_at'
                )
                ->get();

            $current_chapter = $allProgress->where('id', $current_chapter->id)->first();
            $current_chapter->loadVideo()->loadFiles();
    ...
    public function updateChapter(SectionChapter $current_chapter, SectionChapter $new_chapter, Course $course)
    {
        ...
        if ($next_relation == null) {
            ...
            $status = $new_chapter->fileVideo ? 'P' : 'F';
    ...
    {
        return view('aula.viewParticipant.freecourses.finance');
    }
    ...
    public function showFinance(Request $request)
    {
        ...
        $financeUrl = env('API_FINANCE_URL');
        $responseFinance = Http::get($financeUrl, ['ticket' => $request['code']]);
        $data = $responseFinance->json();

        if ($data) {
            $success = true;
            $cuadro1 = collect($data['cuadro1']);
            $html = view('aula.viewParticipant.freecourses.components._financed', compact('data', 'cuadro1'))->render();
        }
