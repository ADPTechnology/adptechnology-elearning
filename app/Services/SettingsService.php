...
class SettingsService
{
    ...
    public function destroyBackground(Config $config, $storage)
    {
        $this->destroyStorageBackground($config, $storage);
    ...
    public function destroyStorageBackground(Config $config, $storage)
    {
        if (Storage::disk($storage)->exists($config->background_url)) {
            Storage::disk($storage)->delete($config->background_url);
        }
    }

    public function updateBackground(Request $request, Config $config, $storage)
    {
        if ($request->hasFile('image')) {
