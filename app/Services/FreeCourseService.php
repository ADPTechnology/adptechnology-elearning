...
use App\Models\{Course, Plan, ProductCertification, SectionChapter, User};
...
class FreeCourseService
{
        ...
        if (Auth::user()) {
            $query = Course::with(
                [
                    ...
                    'plans.subscription' => fn($q) => $q->where('user_id', Auth::user()->id)
                ]
            )
    ...
    public function destroyUserForCourse(ProductCertification $productCertification)
    {
        ...
        $user = $productCertification->user;
        $lastSubscription = $productCertification->subscriptions()->latest()->first();
    ...
    public function storeParticipantFreeCourse($emails, $plan, Course $course)
    {
        $users = $this->getFilteredUsers($emails, $course);

        $plan = Plan::find($plan);
        $this->getCertificationsArray($users, $plan, $course);

        return array("success" => true);
    }

    public function getFilteredUsers($emails, Course $course)
    {
        $usersEmails = $course->userProductCertifications->map(function ($user) {
            return $user->email;
        })->toArray();

        $filteredEmails = array_diff($emails, $usersEmails);

        return User::whereIn('email', $filteredEmails)->get();
    }

    public function getCertificationsArray($users, $plan, Course $course)
    {
